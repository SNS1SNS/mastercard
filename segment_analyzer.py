...
class SegmentAnalyzer:
    ...
    def _classify_segment(self, cluster_data: pd.DataFrame, characteristics: Dict[str, float]) -> Dict[str, str]:
        """Классификация типа сегмента на основе реальных характеристик кластеров"""
        ...
        # Логика классификации на основе реальных паттернов данных
        
        # 1. Спящие клиенты - очень низкая активность (менее 200 транзакций)
        if avg_transactions < 200:
            return {
                'name': '😴 Спящие клиенты',
                'description': 'Очень низкая активность, требуют реактивации',
                'priority': 'Высокий (реактивация)',
                'value': 'Низкая',
                'emoji': '😴'
            }
        
        # 2. VIP клиенты - низкая активность (300-500 транзакций), но высокий средний чек (>25000)
        elif (avg_transactions >= 300 and avg_transactions < 500 and avg_amount > 25000):
            return {
                'name': '💎 VIP клиенты',
                'description': 'Высокий средний чек, премиум сегмент',
                'priority': 'Очень высокий',
                'value': 'Очень высокая',
                'emoji': '💎'
            }
        
        # 3. Путешественники - средняя активность (1500-3000 транзакций), средний чек
        elif (avg_transactions >= 1500 and avg_transactions < 3500):
            return {
                'name': '✈️ Путешественники',
                'description': 'Средняя активность, мобильные клиенты',
                'priority': 'Высокий',
                'value': 'Высокая',
                'emoji': '✈️'
            }
        
        # 4. Активные клиенты - очень высокая активность (более 3500 транзакций)
        elif avg_transactions >= 3500:
            return {
                'name': '⚡ Активные клиенты',
                'description': 'Очень высокая активность, основа клиентской базы',
                'priority': 'Высокий',
                'value': 'Высокая',
                'emoji': '⚡'
            }
        
        # 5. Обычные клиенты - все остальные
        else:
            return {
                'name': '🔄 Обычные клиенты',
                'description': 'Стандартная активность',
                'priority': 'Средний',
                'value': 'Средняя',
                'emoji': '🔄'
            }
